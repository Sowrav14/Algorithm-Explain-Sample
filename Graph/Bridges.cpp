...
struct TECC { // nodes are 0 indexed
    int n, k;                           // k-components after removing bridges.
    vector<vector<int>> g, t;
    vector<bool> used;                  // visited array...
    vector<int> comp, ord, low;         // componenets, discovery_time(ord), least_discovery_time(low).
    ...
    // dfs to find discovery time and low time for nodes.
    ...
    // decomposed tree... or bridge tree...
    void build_tree() {
        t.resize(k);
        for (auto e: br) {
...
void solve(){
    ...
    TECC t(g);
    // think what you need how to get it...
